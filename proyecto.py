...
class GrafoServidores:
    def __init__(self):
        self.grafo = {}  # Diccionario: servidor -> lista de servidores conectados

    def agregar_servidor(self, servidor):
        if servidor not in self.grafo:
            self.grafo[servidor] = []

    def agregar_conexion(self, servidor1, servidor2):
        if servidor1 in self.grafo and servidor2 in self.grafo:
            self.grafo[servidor1].append(servidor2)
            self.grafo[servidor2].append(servidor1)  # Grafo no dirigido
    ...
    def bfs(self, inicio, destino):
        ...
        if inicio not in self.grafo or destino not in self.grafo:
            return None
        visitados = set()
        cola = deque([(inicio, [inicio])])  # (nodo, ruta)
        while cola:
    ...
    def dfs(self, inicio, destino, visitados=None, ruta=None):
        """DFS para encontrar cualquier ruta."""
        ...
        for vecino in self.grafo[inicio]:
            if vecino not in visitados:
                resultado = self.dfs(vecino, destino, visitados, ruta + [vecino])
                if resultado:
                    return resultado
        visitados.remove(inicio)  # Backtrack
        return None
...
class ServidorCorreo:
    ...
    def __init__(self):
        # ... (código existente)
        self.grafo = GrafoServidores()  # Instancia del grafo
        self.grafo.agregar_servidor(self)  # Agregar este servidor

    def conectar_a_servidor(self, otro_servidor):
        """Conecta este servidor a otro."""
        self.grafo.agregar_servidor(otro_servidor)
        self.grafo.agregar_conexion(self, otro_servidor)

    def enviar_mensaje(self, mensaje, usar_bfs=True):
        """Envía mensaje simulando ruta en el grafo."""
        servidor_destino = self._usuarios.get(mensaje.destinatario)
        if not servidor_destino:
            print(f"Error: Usuario no encontrado.")
            return
        # Asumimos que cada usuario está en un servidor; para simplicidad, simulamos ruta
        ruta = self.grafo.bfs(self, servidor_destino) if usar_bfs else self.grafo.dfs(self, servidor_destino)
        if ruta:
            print(f"Ruta de envío: {ruta}")
            # Simular envío: entregar al último servidor en la ruta
            ruta[-1]._usuarios[mensaje.destinatario].recibir_mensaje(mensaje)
        else:
